...
export function checkSimulationNameExists(name) {
    const row = db.prepare('SELECT id FROM simulations WHERE name = ?').get(name);
    return !!row;
}

export function generateUniqueSimulationName(baseName) {
    let newName = baseName;
    let counter = 1;
    while (checkSimulationNameExists(newName)) {
...
export async function resetPortfolio(initialBalance, simulationName = 'Default Simulation', simulationIdToReset = null) {
    if (simulationIdToReset) {
        db.prepare(`DELETE FROM settings WHERE simulation_id = ? `).run(simulationIdToReset);
    ...
    // Cleanup: Keep only the 5 most recent simulations to prevent DB bloat
    const simulationsToKeep = db.prepare(`SELECT id FROM simulations ORDER BY id DESC LIMIT 5`).all().map(row => row.id);

    if (simulationsToKeep.length > 0) {
        const keepIds = simulationsToKeep.join(',');
        db.prepare(`DELETE FROM settings WHERE simulation_id NOT IN(${keepIds})`).run();
        db.prepare(`DELETE FROM portfolio_stats WHERE simulation_id NOT IN(${keepIds})`).run();
        db.prepare(`DELETE FROM agent_logs WHERE simulation_id NOT IN(${keepIds})`).run();
        db.prepare(`DELETE FROM decision_memory WHERE simulation_id NOT IN(${keepIds})`).run();
        db.prepare(`DELETE FROM simulations WHERE id NOT IN(${keepIds})`).run();
    } else {
        db.prepare(`DELETE FROM settings`).run();
        db.prepare(`DELETE FROM portfolio_stats`).run();
        db.prepare(`DELETE FROM agent_logs`).run();
        db.prepare(`DELETE FROM decision_memory`).run();
        db.prepare(`DELETE FROM simulations`).run();
    }

    // Create a new simulation record instead of deleting old ones
    const simStmt = db.prepare(`INSERT INTO simulations(name, status) VALUES(?, 'ACTIVE')`);
    const simInfo = simStmt.run(simulationName);
...
export async function getActiveSimulation() {
    return db.prepare('SELECT * FROM simulations WHERE status = ? ORDER BY id DESC LIMIT 1').get('ACTIVE');
}

export async function getLatestSimulation() {
    return db.prepare('SELECT * FROM simulations ORDER BY id DESC LIMIT 1').get();
}

export async function getAllSimulations() {
    return db.prepare('SELECT * FROM simulations ORDER BY id DESC').all();
}
...
export async function stopSimulation(simulationId) {
    db.prepare('UPDATE simulations SET status = ? WHERE id = ?').run('STOPPED', simulationId);
}

export async function setSimulationStatus(simulationId, status) {
    db.prepare('UPDATE simulations SET status = ? WHERE id = ?').run(status, simulationId);
}

export async function deleteSimulation(id) {
    // Manually delete child records to support older databases that might not have ON DELETE CASCADE
    db.prepare('DELETE FROM settings WHERE simulation_id = ?').run(id);
...
export async function updateSettings(settings, simulationId = null) {
    const stmt = db.prepare(`
      INSERT INTO settings(simulation_id, rpc_url, slippage, openrouter_key, active_model, target_hf, max_gas_claim, data_mode, data_scenario, automation_rules, llm_tools_enabled)
VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    stmt.run(
        simulationId,
